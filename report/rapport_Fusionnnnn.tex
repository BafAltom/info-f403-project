...
\end{figure}
 ...
 Nous obtenons l'AST suivant (la liste des tokens et le parse tree étant trop long pour être ajouté):

\begin{figure}[H]
\fontfamily{pcr}
\begin{lstlisting} 
(name=AST-ROOT, value=)
	(name=Funct-List, value=)
		(name=Funct, value=incr)
			(name=arg, value=arg1)
			(name=Instr-List, value=)
				(name=return, value=)
					(name=OPERATOR, value=ADD)
						(name=VARIABLE, value=arg1)
						(name=INT, value=1)
				(name=Instr, value=END)
	(name=Instr-List, value=)
		(name=Fct-Call, value=PERL-PRIN)
			(name=STRING, value=teste compilateur\n)
		(name=Assign, value=arg1)
			(name=INT, value=1)
		(name=Assign, value=arg1)
			(name=Fct-Call, value=incr)
				(name=VARIABLE, value=arg1)
		(name=Cond, value=)
			(name=OPERATOR, value=EQUIV)
				(name=VARIABLE, value=arg1)
				(name=INT, value=2)
			(name=Instr-List, value=)
				(name=Fct-Call, value=PERL-PRIN)
					(name=STRING, value=increment sucessfull\n)
				(name=Instr, value=END)
			(name=Cond, value=)
				(name=Instr-List, value=)
					(name=Fct-Call, value=PERL-PRIN)
						(name=STRING, value=erreur)
					(name=Instr, value=END)
		(name=Instr, value=END)
\end{lstlisting}
\fontfamily{}
\caption{AST : du code Perl ci-dessus}
\label{lst:exemple_ast}
\end{figure}
 
Ce qui nous donne le code ASM suivant :
\begin{figure}[H]
\fontfamily{pcr}
\begin{lstlisting} 
	.arch armv5te
...
\end{lstlisting}
\fontfamily{}
\caption{ASM : du code Perl ci-dessus}
\label{lst:exemple_ast}
\end{figure}
